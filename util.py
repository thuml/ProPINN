...
def get_perturbation(range, size, type='uniform'):
    if type == 'uniform':
        ...
        x_mesh, t_mesh = np.meshgrid(x_ref, t_ref)
        x_mesh = torch.from_numpy(x_mesh).float()
        t_mesh = torch.from_numpy(t_mesh).float()
    else:
        x_ref = np.random.uniform(-range, range, size)
        t_ref = np.random.uniform(-range, range, size)
        x_mesh, t_mesh = np.meshgrid(x_ref, t_ref)
        x_mesh = torch.from_numpy(x_mesh).float()
        t_mesh = torch.from_numpy(t_mesh).float()
    return torch.cat((x_mesh.reshape(-1)[:, None], t_mesh.reshape(-1)[:, None]), dim=-1)
